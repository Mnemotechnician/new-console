...
public class CopypasteDialog extends BaseDialog {
	...
	public CopypasteDialog() {
		...
		cont.table(main -> {
			...
			main.button("@newconsole.copy", Styles.squareTogglei, () -> {
				if (target == null) {
			...
			main.button("@newconsole.paste", Styles.squareTogglei, () -> {
				if (target == null) {
					Log.warn("No target text area specified for CopypasteDialog");
					return;
				}
			...
			main.button("@newconsole.close", Styles.squareTogglei, this::hide).colspan(2).growX();
		});
	}
